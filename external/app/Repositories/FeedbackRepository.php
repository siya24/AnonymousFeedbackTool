...
class FeedbackRepository {
    ...
    public function getDetailedReport(string $reference): ?array {
        ...
        return [
            ...
            'reporter_feedback_updates' => $this->getAnonymizedSummaryUpdates(
                $reference,
                isset($report['reporter_feedback']) ? (string) $report['reporter_feedback'] : null,
                'reporter_feedback'
            ),
            'anonymized_summary_updates' => $this->getAnonymizedSummaryUpdates(
                $reference,
                isset($report['anonymized_summary']) ? (string) $report['anonymized_summary'] : null
            ),
        ];
    }

    public function getAnonymizedSummaryUpdates(
        string $reference,
        ?string $currentSummary = null,
        string $fieldName = 'anonymized_summary'
    ): array
    {
        $stmt = $this->pdo->prepare(
            'SELECT details, created_at
             FROM audit_logs
             WHERE reference_no = ? AND action = ?
             ORDER BY created_at ASC'
        );
        $stmt->execute([$reference, 'case_updated']);
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            if (!is_array($decoded) || !array_key_exists($fieldName, $decoded)) {
                continue;
            }

            $summary = trim((string) ($decoded[$fieldName] ?? ''));
            if ($summary === '') {
                continue;
            }
        ...
        if ($updates === [] && $currentSummary !== null && trim($currentSummary) !== '') {
            $updates[] = [
                'summary' => trim($currentSummary),
                'created_at' => '',
            ];
        }
    ...
    public function pruneOldAuditLogs(int $retentionDays = 1825): int {
        $stmt = $this->pdo->prepare(
            'DELETE FROM audit_logs WHERE created_at < DATEADD(DAY, -?, CURRENT_TIMESTAMP)'
        );
