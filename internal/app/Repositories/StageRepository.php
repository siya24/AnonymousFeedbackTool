...
final class StageRepository
{
    ...
    public function create(string $name, int $sortOrder = 0, ?string $actorUserId = null): string
    {
        ...
        $stmt = $this->pdo->prepare(
            'INSERT INTO stages (id, name, is_active, created_by_user_id, updated_by_user_id, sort_order, created_at, updated_at) VALUES (?, ?, 1, ?, ?, ?, NOW(), NOW())'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE stages SET name = ?, is_active = ?, sort_order = ?, updated_by_user_id = ?, updated_at = NOW() WHERE id = ?'
        );
